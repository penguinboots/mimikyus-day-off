const { munchlax1, munchlax2, munchlax3, snorlax3 } = require("../floor3mons");

const floor_3 = {
  room_1: {
    name: "Route 35",
    type: "battle",
    opponent: munchlax1,
    treasure: {
      sprite: "",
      reward: {},
    },
    background: `url("/grass_bg.png")`,
    color: "#dda2a7",
    music: "08_munchlax1.mp3",
    intro: `/splashes/vs_munchlax1.png`,
    achievement: null,
    winMessage: "It doesn't matter who you're fighting - it matters if you win!",
    lossMessage: "... By a literal child?",
    next_room: "room_2",
  },
  room_2: {
    name: "Ruins of Alph",
    type: "battle",
    opponent: munchlax2,
    treasure: {
      sprite: "",
      reward: {},
    },
    background: `url("/backgrounds/ruins_of_alph.png")`,
    color: "#7e6651",
    music: "09_munchlax2.mp3",
    intro: `/splashes/vs_munchlax2.png`,
    achievement: null,
    winMessage: "Wow, there were two of them!",
    lossMessage: "Fool me twice...",
    next_room: "room_3",
  },
  room_3: {
    name: "Route 32",
    type: "battle",
    opponent: munchlax3,
    treasure: {
      sprite: "",
      reward: {},
    },
    background: `url("/backgrounds/route_32.png")`,
    color: "#c47979",
    music: "10_munchlax3.mp3",
    intro: `/splashes/vs_munchlax3.png`,
    achievement: null,
    winMessage: "Third time's the charm!",
    lossMessage: "That's a really strong kid...",
    next_room: "room_4",
  },
  room_4: {
    name: "Ecruteak City",
    type: "treasure",
    opponent: null,
    treasure: {
      items:["sitrus-berry"],
      moves:[
        "thunderbolt",
        "swords-dance",
        "drain-punch",
        "dazzling-gleam",
        "shadow-claw",
        "play-rough",
      ]
    },
    background: `url("/backgrounds/ecruteak.png")`,
    color: "#907969",
    music: "11_ecruteak.mp3",
    intro: "",
    achievement: null,
    winMessage: "",
    lossMessage: "",
    next_room: "room_5",
  },
  room_5: {
    name: "Bell Tower",
    type: "battle",
    opponent: snorlax3,
    treasure: {
      sprite: "",
      reward: {},
    },
    background: `url("/backgrounds/bell_tower.png")`,
    color: "#896d5c",
    music: "12_snorlax3.mp3",
    intro: `/splashes/vs_snorlax1.png`,
    achievement: "06_snorlax3",
    winMessage: "Finally caught that slippery thief!",
    lossMessage: "So close... but so far...",
    next_room: "room_6",
  },
  room_6: {
    name: "Cianwood City",
    type: "end",
    opponent: null,
    treasure: {
      sprite: "",
      reward: {},
    },
    background: ``,
    color: "",
    music: "13_endgame.mp3",
    intro: "",
    achievement: null,
    winMessage: "",
    lossMessage: "",
    next_room: null,
  },
  next_floor: null,
}

module.exports = { floor_3 };